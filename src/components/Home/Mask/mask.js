...
export default function Mask({ modelPath }) {
  ...
  useFrame((state) => {
    ...
    meshRef.current.rotation.y = rotationTarget.current.y;
    ...
    meshRef.current.position.y =
      positionTarget.current.y + Math.sin(state.clock.elapsedTime * 0.5) * 0.05;

    const distance = Math.sqrt(
      Math.pow(rotationTarget.current.x, 2) +
        Math.pow(rotationTarget.current.y, 2)
    );
    const scale = 1 + distance * 0.1; // 3.5
