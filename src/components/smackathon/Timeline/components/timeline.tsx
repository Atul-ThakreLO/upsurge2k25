...
export default function Timeline() {
  ...
  return (
    ...
    >
      {/* Death Counter */}
      <div className="absolute top-4 right-4 z-50 bg-black/70 border border-red-700 text-red-500 px-4 py-2 rounded-lg font-mono text-sm shadow-md">
        💀 Deaths: {deathCount}
      </div>
      ...
      <div className="relative z-20 max-w-screen-xl mx-auto px-4 py-1 mb-10 flex flex-col items-center">
        <h2 className="text-red-600 text-5xl font-black tracking-widest font-sans">타임라인</h2>
              ...
              {['left', 'right'].map((side) => {
                ...
                return (
                  <motion.div
                    ...
                    className={`w-20 h-20 relative flex items-center justify-center rounded-md border border-red-500 ring-1 ring-white/30 backdrop-blur-md ${
          ...
          {timelineItems.map((item, idx) => {
            ...
            return (
              <motion.div
                ...
                className="bg-red-500 border-2 border-black rounded-xl p-4 shadow-lg font-mono"
              >
                ...
                {item.time.map((time, i) => (
                  <div key={i} className="text-[0.75rem] leading-snug mb-1">
                    <span className="inline-block bg-[#1f1f1f] text-red-500 font-bold px-2 py-0.5 rounded mr-2 text-[0.65rem]">
                      {time}
                    </span>
