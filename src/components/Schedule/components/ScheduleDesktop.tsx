...
useEffect(() => {
  ...
  style={{
    filter: "drop-shadow(0px 50px 50px #deabba)", // Applying drop-shadow directly to image
  }}
        />
      </div>
  {/* Guy Image on the left */}
{/* Left-side Image + Day 2 Block */}
    ...
    {eventBlocks.map((event, i) => {
      ...
      return (
        <motion.div
          ...
          className="w-64 h-24 bg-pink-500 rounded-lg shadow-2xl flex items-center justify-center font-bold text-lg text-white 
           shadow-pink-500/50 glow-glass-effect animate-glow">
          {event}
        </motion.div>
...
<div className="absolute top-[130vh] right-80 z-20 perspective-1000">
  {eventBlocks.map((event, i) => {
    ...
    return (
      <motion.div
        ...
        className="w-64 h-24 bg-pink-500 rounded-lg shadow-2xl flex items-center justify-center font-bold text-lg text-white ml-auto
          shadow-pink-500/50 glow-glass-effect animate-glow"
      >
        {event}
      </motion.div>
      ...
      <div className="absolute top-0 w-full h-full z-10 pointer-events-none">
        <img
          ...
          className="w-full h-[350vh] object-cover"
          style={{
  ...
  willChange: 'transform',
        ...
        {/* Footer Overlayed on Parallax Background Bottom */}
